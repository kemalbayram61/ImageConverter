...
class ImageLoader:
    ...
    def get_source_image_name_list(self) -> list[str]:
        ...
        for element in all_elements:
            if element.endswith(self.__source_image_extension.value) or element.endswith(self.__source_image_extension.value.upper()):
    ...
    def get_source_image_count(self) -> int:
        ...
        for element in all_elements:
            if element.endswith(self.__source_image_extension.value) or element.endswith(self.__source_image_extension.value.upper()):
                response.append(element)
        print("Kaynakta {0} adet resim bulundu.".format(len(response)))
        return len(response)
    ...
    def get_image_list(self, index) -> list[Image]:
        ...
        for image_name in image_name_list:
            ...
            print("{0} isimli dosya okundu.".format(image_name))
        return response
